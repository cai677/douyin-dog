...
description: 将抖音带货视频下载、抽帧、拆成可复拍分镜，并归档为带图片的飞书六段式文档。用户提供“编号-产品名 + 抖音链接”或本地带货视频并要求拆片、分镜、逐句文案时间轴或飞书归档时使用。
---

# 带货短视频拆解

交付结果是一条视频对应一份可直接复拍的飞书文档。流程为：获取视频 → 抽帧与镜头检测 → 逐帧分析 → 生成固定六段式文档 → 云端回查。
...
6. 六、逐句文案时间轴：表格列固定为序号 / 时间 / 文案；每行只放一句自然短句，必须有可解析起止秒数，例如 `0-1.2s`

不要增加画面描述、动作、拍多久、拍摄建议等表格列。表格内每个镜头通常只放一张代表帧，并保持图片小尺寸。

若下一步要生成剪映草稿，顶部摘要必须包含 `视频时长 | XX.XX 秒`，第五段表头必须保留 `时间` 和 `台词`，第六段表头必须保留 `时间` 和 `文案`。优先让下游读取第六段逐句文案时间轴；第六段缺失时才回退第五段完整分镜脚本。
...
后续与 `jianying-voice-draft` 对接时，飞书文档格式固定为当前六段式结构，尤其是第五段“完整分镜脚本（含画面帧）”和第六段“逐句文案时间轴”。第五段表格列顺序固定为：镜号 / 时间 / 时长 / 景别 / 台词 / 画面帧 / 音效。第六段表格列顺序固定为：序号 / 时间 / 文案。

下游可优先读取第六段逐句文案时间轴里的 `时间` 和 `文案` 来生成朗读音频和字幕；如果第六段缺失，再回退读取第五段完整分镜脚本里的 `时间` 和 `台词`。没有单独 `视频时长 | XX.XX 秒` 字段时，用最后一条时间段结束点作为总时长。
