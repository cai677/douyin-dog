...
- 飞书文档必须包含可解析文案表格。优先读取表头里的 `时间` 和 `台词`/`口播文案`/`字幕`/`文案` 列；兼容旧版无表头四列表格。
- 若文档来自 `daihuo-video-breakdown`，可直接读取第五段“完整分镜脚本”表格；该表格列顺序固定为：镜号 / 时间 / 时长 / 景别 / 台词 / 画面帧 / 音效。
- 如果文档没有单独 `视频时长 | XX.XX 秒` 字段，用完整分镜脚本最后一行 `时间` 的结束点作为总时长；如果解析不到时间列或台词列，先回到拆解文档补齐交接字段。
- 连续重复文案只朗读一次；非连续重复保留。
...
5. 草稿总时长等于飞书文档视频总时长；若文档未单独标注视频时长，则等于完整分镜脚本最后一行结束时间。
6. 连续重复文案没有重复朗读。
7. 每句起声没有明显句首空白。
...
`daihuo-video-breakdown` 后续固定输出当前五段式飞书文档格式时，`jianying-voice-draft` 直接读取第五段“完整分镜脚本（含画面帧）”表格。表格列顺序固定为：镜号 / 时间 / 时长 / 景别 / 台词 / 画面帧 / 音效。

当文档没有单独的 `视频时长 | XX.XX 秒` 字段时，不再判定为错误；用完整分镜脚本最后一行 `时间` 的结束点作为剪映草稿总时长。
