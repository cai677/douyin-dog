...
- 可解析文案表格。优先使用 `daihuo-video-breakdown` 第六段“逐句文案时间轴”表格；如果缺失，再回退旧版 `文案+画面对照表` 或第五段“完整分镜脚本”表格
- 表格字段至少包含：
  - 时间
  - 文案、台词、口播文案、字幕中的任意一种表头
...
1. 从飞书表格逐行提取文案；六段式上游文档优先读取第六段“逐句文案时间轴”的 `时间` 和 `文案`。
2. 连续重复文案只保留一次朗读。
