...
if sys.argv[1]=='audio':
 ...
 raw=json.loads((O/'voice/voice-report.json').read_text(encoding='utf-8'));parts=[];cursor=0;rows=[];short=[]
 for e in raw['segments']:
  ...
  p=O/'voice'/(e['id']+'-clean.wav');sf.write(p,y,sr,subtype='PCM_16');d=len(y)/sr
 ...
 sf.write(O/'voice/final_voice-clean.wav',np.concatenate(parts),sr,subtype='PCM_16');sf.write(O/'voice/final_voice-clean.mp3',np.concatenate(parts),sr,format='MP3')
 sf.write(O/'action-sample-voice.wav',np.concatenate(short),sr,subtype='PCM_16')
 save(O/'voice/voice-report-clean.json',dict(schema_version=2,provider='doubao_tts',speaker=raw['speaker'],raw_duration_seconds=raw['actual_duration_seconds'],actual_duration_seconds=cursor,voice_speed_changed=False,cleanup='10ms RMS; 60ms head 90ms tail retained; internal pauses unchanged',segments=rows))
 print('Clean voice:',cursor)
else:
 import cv2,numpy as np
 from PIL import Image,ImageDraw,ImageFont
 report=json.loads((O/'voice/voice-report-clean.json').read_text(encoding='utf-8'));cat=[json.loads(x) for x in (R/'assets/catalog/clips.jsonl').read_text(encoding='utf-8').splitlines()]
