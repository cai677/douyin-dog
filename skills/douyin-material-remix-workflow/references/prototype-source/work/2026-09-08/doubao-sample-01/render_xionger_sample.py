...
O=Path(__file__).resolve().parent;T=O.parent;R=T.parents[1];V=O/'voice-zh_male_xionger_mars_bigtts-rate50';E=O/'full-xionger-rate50';E.mkdir(exist_ok=True)
report=json.loads((V/'voice-report.json').read_text(encoding='utf-8'));matches=json.loads((T/'matches.json').read_text(encoding='utf-8'));font=ImageFont.truetype('C:/Windows/Fonts/msyh.ttc',48);big=ImageFont.truetype('C:/Windows/Fonts/msyh.ttc',60)
def fit(frame):
